...
└── src/
    ...
    ├── index.css                   Tailwind layers + Kinetic Dark Redux primitives + tier-effect keyframes + exercise thumbnail/GIF demo styles
    ...
    │   ├── ExerciseThumb.jsx       shared exercise thumbnail: exercise `gif` URL or cached media URL, then emoji/fallback pose
    ...
    └── pages/
        ...
        ├── WorkoutLogger.jsx       Liftit active session: timer/progress header, tappable exercise GIF/thumbnail cards, prev/lbs/reps set rows, FinishModal asks for pump pic.
...
- **Exercise media import script:** added `scripts/fetch-gifs.mjs` for `free-exercise-db`. It copies matching exercise demo images into `public/assets/exercises`; thumbnails now also try `.jpg` and `.png` static files, because that source repo mostly ships JPG frames rather than actual GIFs. Current mapping imports 80 exercises; Pendlay Row and Sumo Squat still need manual/demo replacements if desired.
